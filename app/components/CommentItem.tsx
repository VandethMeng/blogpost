"use client";

import { Comment } from "@/types/post";

type User = {
  username: string;
  role?: string;
};

type CommentItemProps = {
  comment: Comment;
  postId: string;
  currentUser: User | null;
  onDelete: () => void;
};

export default function CommentItem({ comment, postId, currentUser, onDelete }: CommentItemProps) {
  const handleDelete = async () => {
    if (!confirm("Delete this comment?")) return;
    
    try {
      const res = await fetch(`/api/posts/${postId}/comments`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ commentId: comment.id }),
      });
      
      if (res.ok) onDelete();
    } catch (error) {
      console.error("Failed to delete comment:", error);
    }
  };

  return (
    <div className="border-l-2 border-gray-300 pl-2 mb-2">
      <div className="flex justify-between items-start">
        <div className="flex-1">
          <p className="text-sm font-semibold">{comment.author}</p>
          <p className="text-gray-700">{comment.content}</p>
          <p className="text-xs text-gray-500">{new Date(comment.createdAt).toLocaleString()}</p>
        </div>
        {/* Show delete button only if user is logged in and owns the comment (or is admin) */}
        {currentUser && (currentUser.username === comment.author || currentUser.role === "admin") && (
          <button
            onClick={handleDelete}
            className="text-red-600 hover:text-red-800 transition p-1 ml-2"
            title="Delete comment"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
              <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
            </svg>
          </button>
        )}
      </div>
    </div>
  );
}
